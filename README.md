...
- **WorldSystem probe now logs vfunc fingerprints** - With `dump_world_system_probe = true`, each sibling line in `cdcoop_world_probe.log` now includes the first 4 vtable function RVAs alongside the vtable RVA itself. That's the behavioural fingerprint a community submitter needs to cross-reference against an x64dbg / IDA / Ghidra dump to identify which sibling is the quest / cutscene / world-object manager — significantly more useful than the bare vtable RVA we logged in 0.2.2.
