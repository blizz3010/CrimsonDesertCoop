...
| 9 | **MEDIUM** | Mount HP (Dragon) | Dragon mount health, stamina, and flight timer offsets | **Unresolved** - confirmed float type (not int64*1000) | FearLess pages 14-16: dragon HP hard to find. May be float not int4. Community tried 4-byte, float, ALL value types without success. Separate entity pool from ground mounts? |
...
- **Dragon Timer Offset** - r13+0x160 (float, from community CT - needs hook point AOB)
- **Resistance Attrs** - AOB found in bbfox0703 CT (injection point identified, struct layout TBD)
